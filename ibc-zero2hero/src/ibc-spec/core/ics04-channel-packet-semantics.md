...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 4 | Channel & Packet Semantics | draft | IBC/TAO | instantiation | 2, 3, 5, 24 | Christopher Goes <cwgoes@tendermint.com> | 2019-03-07 | 2019-08-25 |
...
`ConsensusState` is as defined in [ICS 2](https://github.com/cosmos/ibc/blob/main/spec/core/ics-002-client-semantics).

`Connection` is as defined in [ICS 3](https://github.com/cosmos/ibc/blob/main/spec/core/ics-003-connection-semantics).

`Port` and `authenticateCapability` are as defined in [ICS 5](https://github.com/cosmos/ibc/blob/main/spec/core/ics-005-port-allocation).
...
`Identifier`, `get`, `set`, `delete`, `getCurrentHeight`, and module-system related primitives are as defined in [ICS 24](https://github.com/cosmos/ibc/blob/main/spec/core/ics-024-host-requirements).
...
![https://github.com/cosmos/ibc/raw/main/spec/core/ics-004-channel-and-packet-semantics/dataflow.png](https://github.com/cosmos/ibc/raw/main/spec/core/ics-004-channel-and-packet-semantics/dataflow.png)
...
Packet receipt data are stored under the `packetReceiptPath`. In the case of a successful receive, the destination chain writes a sentinel success value of `SUCCESSFUL_RECEIPT`. Some channel types MAY write a sentinel timeout value `TIMEOUT_RECEIPT` if the packet is received after the specified timeout.
...
![https://github.com/cosmos/ibc/raw/main/spec/core/ics-004-channel-and-packet-semantics/channel-state-machine.png](https://github.com/cosmos/ibc/raw/main/spec/core/ics-004-channel-and-packet-semantics/channel-state-machine.png)
...
![https://github.com/cosmos/ibc/raw/main/spec/core/ics-004-channel-and-packet-semantics/packet-state-machine.png](https://github.com/cosmos/ibc/raw/main/spec/core/ics-004-channel-and-packet-semantics/packet-state-machine.png)
...
The module can interface with the IBC handler through [ICS 25](https://github.com/cosmos/ibc/blob/main/spec/core/ics-025-handler-interface) or [ICS 26](https://github.com/cosmos/ibc/blob/main/spec/core/ics-026-routing-module).

1. Initial client & port setup, in any order
    1. Client created on *A* for *B* (see [ICS 2](https://github.com/cosmos/ibc/blob/main/spec/core/ics-002-client-semantics))
    2. Client created on *B* for *A* (see [ICS 2](https://github.com/cosmos/ibc/blob/main/spec/core/ics-002-client-semantics))
    3. Module *1* binds to a port (see [ICS 5](https://github.com/cosmos/ibc/blob/main/spec/core/ics-005-port-allocation))
    4. Module *2* binds to a port (see [ICS 5](https://github.com/cosmos/ibc/blob/main/spec/core/ics-005-port-allocation)), which is communicated out-of-band to module *1*
2. Establishment of a connection & channel, optimistic send, in order
    1. Connection opening handshake started from *A* to *B* by module *1* (see [ICS 3](https://github.com/cosmos/ibc/blob/main/spec/core/ics-003-connection-semantics))
...
3. Successful completion of handshakes (if either handshake fails, the connection/channel can be closed & the packet timed-out)
    1. Connection opening handshake completes successfully (see [ICS 3](https://github.com/cosmos/ibc/blob/main/spec/core/ics-003-connection-semantics)) (this will require participation of a relayer process)
...
![https://github.com/cosmos/ibc/raw/main/spec/core/ics-004-channel-and-packet-semantics/packet-transit.png](https://github.com/cosmos/ibc/raw/main/spec/core/ics-004-channel-and-packet-semantics/packet-transit.png)
...
- Checks that the calling module owns the sending port (see [ICS 5](https://github.com/cosmos/ibc/blob/main/spec/core/ics-005-port-allocation))
...
We pass the `relayer` address just as in [Receiving packets](https://github.com/cosmos/ibc/tree/main/spec/core/ics-004-channel-and-packet-semantics#receiving-packets) to allow for possible incentivization here as well.
...
If an application uses a different format for acknowledgement bytes, it MUST not deserialise to a valid protobuf message of this format. Note that all packets contain exactly one non-empty field, and it must be result or error. The field numbers 21 and 22 were explicitly chosen to avoid accidental conflicts with other protobuf message formats used for acknowledgements. The first byte of any message with this format will be the non-ASCII values `0xaa` (result) or `0xb2` (error).

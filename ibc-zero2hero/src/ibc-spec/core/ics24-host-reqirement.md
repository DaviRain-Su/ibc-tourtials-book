# Host State Machine Requirements

| ics | title | stage | category | kind | requires | required-by | author | created | modified |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 24 | Host State Machine Requirements | draft | IBC/TAO | interface | 23 | 2, 3, 4, 5, 18 | Christopher Goes <cwgoes@tendermint.com> | 2019-04-16 | 2022-07-05 |
...
- MUST write to a key/value store whose data can be externally proved with a vector commitment as defined in [ICS 23](https://github.com/cosmos/ibc/blob/main/spec/core/ics-023-vector-commitments).
...
Parts of the private store MAY safely be used for other purposes as long as the IBC handler has exclusive access to the specific keys required. Keys used in the private store MAY safely vary as long as there exists a bipartite mapping between the key formats defined herein and the ones actually used in the private store implementation.

Note that the client-related paths listed below reflect the Tendermint client as defined in [ICS 7](https://github.com/cosmos/ibc/blob/main/spec/client/ics-007-tendermint-client) and may vary for other client types.
...
Host state machines MUST define a unique `ConsensusState` type fulfilling the requirements of [ICS 2](https://github.com/cosmos/ibc/blob/main/spec/core/ics-002-client-semantics), with a canonical binary serialisation.
...
Host state machines MUST define a unique `ClientState` type fulfilling the requirements of [ICS 2](https://github.com/cosmos/ibc/blob/main/spec/core/ics-002-client-semantics).
...
`validateSelfClient` validates the client parameters for a client of the host chain. For example, below is the implementation for Tendermint hosts, using `ClientState` as defined in [ICS 7](https://github.com/cosmos/ibc/blob/main/spec/client/ics-007-tendermint-client):
...
This permissioning can be implemented with unique references (object capabilities) for each port (a la the Cosmos SDK), with source authentication (a la Ethereum), or with some other method of access control, in any case enforced by the host state machine. See [ICS 5](https://github.com/cosmos/ibc/blob/main/spec/core/ics-005-port-allocation) for details.
...
Host state machines which implement the routing module MAY define a `submitDatagram` function to submit datagrams[1](https://github.com/cosmos/ibc/tree/main/spec/core/ics-024-host-requirements#footnote1), which will be included in transactions, directly to the routing module (defined in [ICS 26](https://github.com/cosmos/ibc/blob/main/spec/core/ics-026-routing-module)):
